...
class ControllerPaymentVeritransbcains extends Controller {
  ...
  public function index() {
    ...
    $data['pay_type'] = $this->config->get('veritransbcains_payment_type');
    $data['text_loading'] = $this->language->get('text_loading');

    $data['process_order'] = $this->url->link('payment/veritransbcains/process_order');
  ...
  public function process_order() {
    $this->load->model('payment/veritransbcains');
    ...
    $this->model_checkout_order->addOrderHistory($this->session->data['order_id'],
        $this->config->get('veritransbcains_vtweb_challenge_mapping'));
    ...
    if ($this->config->get('config_currency') != 'IDR') {
      ...
      else if ($this->config->get('veritransbcains_currency_conversion') > 0) {
        foreach ($item_details as &$item) {
          $item['price'] = intval($item['price']
              * $this->config->get('veritransbcains_currency_conversion'));
        }
        unset($item);
    ...
    Veritrans_Config::$serverKey = $this->config->
        get('veritransbcains_server_key_v2');

    Veritrans_Config::$isProduction =
        $this->config->get('veritransbcains_environment') == 'production'
        ? true : false;
    ...
    $threshold = $this->config->get('veritransbcains_threshold');
    ...
    try {
      ...
      $bins = $this->config->get('veritransbcains_bin_number');
      ...
      if ($this->config->get('veritransbcains_installment_option') == 'all_product')
      {
        $payment_options = array(
          ...
          $term = $this->config->get('veritransbcains_installment_bca_term');
          $term_array = explode(',', $term);
          $installment_terms['bca'] = $term_array;          
          $payment_options['installment']['installment_terms'] = $installment_terms;

        if ($transaction_details['gross_amount'] >= $threshold) {
          $payloads['vtweb']['payment_options'] = $payment_options;
        }
      }
      else if ($this->config->get('veritransbcains_installment_option') == 'certain_product')
      {
      ...
      if ($is_installment && $this->config->get('veritransbcains_installment_option') != 'all_product') {
      ...
      else if ($this->config->get('veritransbcains_installment_option') == 'all_product' &&
          ($transaction_details['gross_amount'] < $threshold)) {
  ...
  */
  public function failure() {
    ...
    if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/veritransbcains_checkout_failure.tpl')) {
      $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/payment/veritransbcains_checkout_failure.tpl', $data));
    } else {
      $this->response->setOutput($this->load->view('default/template/payment/veritransbcains_checkout_failure.tpl', $data));
    }
  }
}
