...
class ControllerPaymentVeritransbca extends Controller {
  ...
  public function index() {
    ...
    $data['pay_type'] = $this->config->get('veritransbca_payment_type');
    $data['text_loading'] = $this->language->get('text_loading');

    $data['process_order'] = $this->url->link('payment/veritransbca/process_order');
  ...
  public function process_order() {
    ...
    $this->model_checkout_order->addOrderHistory($this->session->data['order_id'],
        $this->config->get('veritransbca_vtweb_challenge_mapping'));
    ...
    if ($this->config->get('config_currency') != 'IDR') {
      ...
      else if ($this->config->get('veritransbca_currency_conversion') > 0) {
        foreach ($item_details as &$item) {
          $item['price'] = intval($item['price']
              * $this->config->get('veritransbca_currency_conversion'));
        }
        unset($item);
    ...
    Veritrans_Config::$serverKey = $this->config->
        get('veritransbca_server_key_v2');

    Veritrans_Config::$isProduction =
        $this->config->get('veritransbca_environment') == 'production'
        ? true : false;
  ...
  public function landing_redir() {
    ...
    if( isset($_GET['order_id']) && isset($_GET['transaction_status']) && ($_GET['transaction_status'] == 'capture' || $_GET['transaction_status'] == 'pending' || $_GET['transaction_status'] == 'settlement')) {
      ...
      $redirUrl = $this->url->link('payment/veritransbca/failure','','SSL');
      $this->response->redirect($redirUrl);
  ...
  */
  public function failure() {
    ...
    if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/veritransbca_checkout_failure.tpl')) {
      $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/payment/veritransbca_checkout_failure.tpl', $data));
    } else {
      $this->response->setOutput($this->load->view('default/template/payment/veritransbca_checkout_failure.tpl', $data));
    }
  }
}
