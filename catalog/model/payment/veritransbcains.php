...
class ModelPaymentVeritransbcains extends Model {
  ...
  public function getMethod($address, $total) {
		...
		if ($this->config->get('veritransbcains_total') > 0 && $this->config->get('veritransbcains_total') > $total) {
			$status = false;
		} elseif (!$this->config->get('veritransbcains_geo_zone_id')) {
			$status = true;
		...
		if ($status) {
      $method_data = array(
        'code'       => 'veritransbcains',
        'title'      => $this->config->get('veritransbcains_display_name'),
		'sort_order' => $this->config->get('veritransbcains_sort_order'),
		'terms' 	 => ''
      );
    }
