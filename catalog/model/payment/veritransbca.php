...
class ModelPaymentVeritransbca extends Model {
  ...
  public function getMethod($address, $total) {
		...
		if ($this->config->get('veritransbca_total') > 0 && $this->config->get('veritransbca_total') > $total) {
			$status = false;
		} elseif (!$this->config->get('veritransbca_geo_zone_id')) {
			$status = true;
      ...
      $method_data = array(
        'code'       => 'veritransbca',
        'title'      => $this->config->get('veritransbca_display_name'),
		'sort_order' => $this->config->get('veritransbca_sort_order'),
		'terms' 	 => ''
      );
    }
