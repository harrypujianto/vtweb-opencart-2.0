...
class ModelPaymentVeritransmandiri extends Model {
  ...
  public function getMethod($address, $total) {
		...
		if ($this->config->get('veritransmandiri_total') > 0 && $this->config->get('veritransmandiri_total') > $total) {
			$status = false;
		} elseif (!$this->config->get('veritransmandiri_geo_zone_id')) {
			$status = true;
		...
		if ($status) {
      $method_data = array(
        'code'       => 'veritransmandiri',
        'title'      => $this->config->get('veritransmandiri_display_name'),
		'sort_order' => $this->config->get('veritransmandiri_sort_order'),
		'terms' 	 => ''
      );
    }
