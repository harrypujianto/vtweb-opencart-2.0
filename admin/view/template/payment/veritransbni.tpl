			...
			<div class="panel-body">
				  <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form" class="form-horizontal">
						...
						<div class="form-group">
							...
							<div class="col-sm-3">
							  <select name="veritransbni_status" id="input-mode" class="form-control">
								<?php $options = array('1' => $text_enabled, '0' => $text_disabled) ?>
								<?php foreach ($options as $key => $value): ?>
								  <option value="<?php echo $key ?>" <?php if ($key == $veritransbni_status) echo 'selected' ?> ><?php echo $value ?></option>
								<?php endforeach ?>
							  </select>
							</div>
						</div>
						...
						<div class="form-group required">
							...
							<div class="col-sm-3">
							  <input type="text" name="veritransbni_display_name" value="<?php echo $veritransbni_display_name; ?>" id="input-merchant-id" class="form-control" />
							</div>
							<div class="col-sm-3">
						...
						<div class="form-group v2_settings sensitive required">
							...
							<div class="col-sm-3">
								<select name="veritransbni_environment" id="input-mode" class="form-control">
									<?php $options = array('development' => 'Sandbox', 'production' => 'Production') ?>
									<?php foreach ($options as $key => $value): ?>
									<option value="<?php echo $key ?>" <?php if ($key == $veritransbni_environment) echo 'selected' ?> ><?php echo $value ?></option>
									<?php endforeach ?>
								</select>
							</div>
							<div class="col-sm-3">
						...
						<div class="form-group required v2_settings sensitive">
							...
							<div class="col-sm-3">
							  <input type="text" name="veritransbni_server_key_v2" value="<?php echo $veritransbni_server_key_v2; ?>" id="input-server-key" class="form-control" />
							</div>
							<div class="col-sm-3">
						...
						<label class="col-sm-2 control-label" for="input-merchant-id">Enable Installment</label>
							<div class="col-sm-3">
								<select name="veritransbni_installment_option" id="installmentOption" class="form-control">
								<?php $options = array('off' => 'Off', 'all_product' => 'All Products', 'certain_product' => 'Certain Product') ?>
								<?php foreach ($options as $key => $value): ?>
								<option value="<?php echo $key ?>" <?php if ($key == $veritransbni_installment_option) echo 'selected' ?> ><?php echo $value ?></option>
								<?php endforeach ?>
								</select>
							</div>
						</div>
						...
						<div class="form-group required v2_settings sensitive">
							<label class="col-sm-2 control-label" for="input-installment-term"><?php echo $entry_installment_bni_term; ?></label>
							<div class="col-sm-3">
								<input type="text" name="veritransbni_installment_bni_term" value="<?php echo $veritransbni_installment_bni_term; ?>" id="input-installment-term" class="form-control" />
							  <span>use comma (,) to separate each term. e.g : 3,6,12</span>
							</div>
							<div class="col-sm-3">
								<?php if (isset($error['installment_bni_term'])) { ?>
								<div class="col-sm-3"> <?php echo $error['installment_bni_term']; ?> </div>
								<?php } ?>
							</div>
						</div>
						<!-- Installment term -->
						...
						<div class="form-group required v2_settings sensitive">
							...
							<div class="col-sm-3">
							  <input type="textarea" name="veritransbni_bin_number" value="<?php echo $veritransbni_bin_number; ?>" id="input-bin-number" class="form-control" />
					...
					<?php foreach (array('vtweb_success_mapping', 'vtweb_failure_mapping', 'vtweb_challenge_mapping') as $status): ?>
						...
						<label class="col-sm-2 control-label" for="input-merchant-id"><?php echo ${'entry_' . $status} ?></label>
							<div class="col-sm-3">
								<select name="<?php echo 'veritransbni_' . $status ?>" id="veritransbniPaymentType" class="form-control">
							  <?php foreach ($order_statuses as $option): ?>
								<option value="<?php echo $option['order_status_id'] ?>" <?php if ($option['order_status_id'] == ${'veritransbni_' . $status}) echo 'selected' ?> ><?php echo $option['name'] ?></option>
							  <?php endforeach ?>
								</select>
							</div>
						</div>
					...
					<!-- VTWeb Mapping -->
						...
						<div class="form-group required">
							...
							<div class="col-sm-3">
							  <input type="text" name="veritransbni_currency_conversion" value="<?php echo $veritransbni_currency_conversion; ?>" class="form-control" />
					...
					<div class="form-group v2_vtweb_settings">
						<label class="col-sm-2 control-label" for="input-merchant-id"><?php echo $entry_geo_zone; ?></label>
							<div class="col-sm-3">
								<select name="veritransbni_geo_zone_id"  class="form-control">
								<option value="0"><?php echo $text_all_zones; ?></option>
								<?php foreach ($geo_zones as $geo_zone) { ?>
									<?php if ($geo_zone['geo_zone_id'] == $veritransbni_geo_zone_id) { ?>
						...
						<div class="form-group">
							...
							<div class="col-sm-1">
							  <input size="1" type="text" name="veritransbni_sort_order" value="<?php echo $veritransbni_sort_order; ?>" class="form-control" />
							</div>
						</div>
						<div>
